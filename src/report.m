...
resnorm_TC = norm(res_S,'fro')./(norm(S(inz,inx)/dt,'fro')+TINY) ...
           + norm(res_C,'fro')./(norm(C(inz,inx)/dt,'fro')+TINY) ...
           + norm(res_V,'fro')./(norm(V(inz,inx)/dt,'fro')+TINY) ...
           + norm(res_X.*(x(inz,inx)>10*TINY).*(m(inz,inx)>10*TINY),'fro')./(norm(X(inz,inx)/dt,'fro')+TINY) ...
           + norm(res_F.*(f(inz,inx)>10*TINY).*(m(inz,inx)>10*TINY),'fro')./(norm(F(inz,inx)/dt,'fro')+TINY) ...
           + norm(res_rho,'fro')./(norm(rho(inz,inx)/dt,'fro')+TINY);

resnorm = 0*resnorm_VP + resnorm_TC;
