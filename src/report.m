...
resnorm    = norm(upd_W  (:))./(norm(W  (:))+TINY) ...
           + norm(upd_P  (:))./(norm(P  (:))+TINY) ...
           + norm(upd_S  (:))./(norm(S  (:))+TINY) ...
           + norm(upd_C  (:))./(norm(C  (:))+TINY) ...
           + norm(upd_rho(:))./(norm(rho(:))+TINY) ...
           + norm(upd_X  (:).*hasx(:))./(norm(X  (:))+TINY) ...
           + norm(upd_M  (:).*hasm(:))./(norm(M  (:))+TINY) ...
           + norm(upd_F  (:).*hasf(:))./(norm(F  (:))+TINY);
