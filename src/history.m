...
hist.sumS(stp) = sum(sum( (S(2:end-1,2:end-1) + S0(2:end-1,2:end-1))*h*h*1));  % [J]
...
dsumMdt = sum(  rho(2,2:end-1).*x(2,2:end-1).*Wx(1,2:end-1)*h*1) - sum(rho(end-1,2:end-1).*x(end-1,2:end-1).*Wx(end,2:end-1)*h*1) ...
        + sum(  rho(2:end-1,2).*x(2:end-1,2).*Ux(2:end-1,1)*h*1) - sum(rho(2:end-1,end-1).*x(2:end-1,end-1).*Ux(2:end-1,end)*h*1) ...
        + sum(  rho(2,2:end-1).*f(2,2:end-1).*Wf(1,2:end-1)*h*1) - sum(rho(end-1,2:end-1).*f(end-1,2:end-1).*Wf(end,2:end-1)*h*1) ...
