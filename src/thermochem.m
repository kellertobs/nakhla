...
if step > 0
    ...
    dtk = min((h/2)^2./kT(:).*rho(:).*Cp(:));                              % diffusive time step size
    ...
    % update temperature
    if ~isotherm
        advn_T = advection(T,U,WT,h,ADVN,'adv');                           % get advection term
                
        qTz    = - (kT(1:end-1,:)+kT(2:end,:))./2 .* ddz(T,h);             % z-heat diffusion flux
        qTx    = - (kT(:,1:end-1)+kT(:,2:end))./2 .* ddx(T,h);             % x-heat diffusion flux
        diff_T(2:end-1,2:end-1) = (- ddz(qTz(:,2:end-1),h) ...             % heat diffusion
                                   - ddx(qTx(2:end-1,:),h)) ...
                                   ./rho(2:end-1,2:end-1)./Cp(2:end-1,2:end-1);
        ...
        cool = (Twall-T)./tau_T .* coolshape;
        
        lath   = Gx.*LHx./Cp - Gf.*LHf./Cp;                                % latent to sensible heat conversion rate
        
        dTdt   = - advn_T + diff_T + cool + lath;                          % total rate of change
        
        T = To + (theta.*dTdt + (1-theta).*dTdto).*dt;                     % explicit update of temperature
        T([1 end],:) = T([2 end-1],:);                                     % apply boundary conditions
        T(:,[1 end]) = T(:,[2 end-1]);
    end
    
    % update composition
    if ~isochem
        advn_c = advection(x.*cx,Ux,Wx,h,ADVN,'flx') + advection(m.*cm,Um,Wm,h,ADVN,'flx'); % advection
                
        qcz   = - kc.*(mu(1:end-1,:)+mu(2:end,:))/2 .* ddz(c,h);           % z-volume chemical flux
        qcx   = - kc.*(mu(:,1:end-1)+mu(:,2:end))/2 .* ddx(c,h);           % x-volume chemical flux
        diff_c(2:end-1,2:end-1) = - ddz(qcz(:,2:end-1),h) ...              % chemical diffusion
                                  - ddx(qcx(2:end-1,:),h);
        
        assim = zeros(size(c));
        if ~isnan(ctop); assim = assim + (ctop-c)./tau_c.* exp((-ZZ)/dw); end % impose top assimilation rate
        
        dcdt = - advn_c + diff_c + assim;                                  % total rate of change
        
        c = co + (theta.*dcdt + (1-theta).*dcdto).*dt;                     % explicit update of composition
        c = min(cphs1-TINY,max(cphs0+TINY,c));                             % enforce min/max limits
        c([1 end],:) = c([2 end-1],:);                                     % apply boundary conditions
        c(:,[1 end]) = c(:,[2 end-1]);
        
        
        % update volatile content
        advn_v = advection(f.*vf,Uf,Wf,h,ADVN,'flx') + advection(m.*vm,Um,Wm,h,ADVN,'flx'); % advection
        
        qvz   = - kc.*(mu(1:end-1,:)+mu(2:end,:))/2 .* ddz(v,h);           % z-volume chemical flux
        qvx   = - kc.*(mu(:,1:end-1)+mu(:,2:end))/2 .* ddx(v,h);           % x-volume chemical flux
        diff_v(2:end-1,2:end-1) = - ddz(qvz(:,2:end-1),h) ...              % volatile diffusion
                                  - ddx(qvx(2:end-1,:),h);
        
        degas = zeros(size(f));
        if ~isnan(ftop); degas = degas + 100 .* min(0,(ftop-f)./tau_f.* exp((-ZZ)/dw)); end  % impose top outgassing rate
        
        dvdt = - advn_v + diff_v + degas;                                  % total rate of change
        
        v = vo + (theta.*dvdt + (1-theta).*dvdto).*dt;                     % explicit update of composition
        v = min(100-TINY,max(TINY,v));                                     % enforce min limit
...
%                                                Pt,Tphs0,Tphs1,cphs0,cphs1,perT,perCx,perCm,clap,dTH2O,PhDg);
    ...
    if diseq
        ...
        vf  = vfq;
        vm  = max(v+TINY,min(100 -TINY,(v - f.*vf)./(1-f-x)));
    ...
    if step == 1; dTdto = dTdt;  dcdto = dcdt;  dvdto = dvdt;  dxdto = dxdt;  dfdto = dfdt; end
end
