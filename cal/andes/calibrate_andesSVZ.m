...
for ic = nc
    if isfield(MAG(ic).OUT.PhaseProps,'ol')
        ...
        DATA.X = [DATA.X;MAG(ic).OUT.OxideFract.ol(hasolv,[cal.Si,cal.Fe,cal.Mg]).*100];
...
for ic = nc
    if isfield(MAG(ic).OUT.PhaseProps,'spn')
        ...
        DATA.X = [DATA.X;MAG(ic).OUT.OxideFract.spn(hasoxs,[cal.Fe,cal.Ti,cal.Al,cal.Mg]).*100];
...
for ic = nc
    ...
    DATA.X = [DATA.X;MAG(ic).OUT.OxideFract.opx(hasopx,[cal.Si,cal.Al,cal.Fe,cal.Mg,cal.Ca]).*100];
...
cal_andesSVZ;  % load melt model calibration
DATA.PRJCT  = 'andesSVZ';
DATA.VNAMES = cal.oxdStr([cal.Si,cal.Al,cal.Fe,cal.Mg,cal.Ca,cal.Na,cal.K]);
DATA.SNAMES = {};
DATA.X      = [];
DATA.T      = [];
for ic = nc
    ...
    DATA.X = [DATA.X;MAG(ic).OUT.OxideFract.cpx(hascpx,[cal.Si,cal.Al,cal.Fe,cal.Mg,cal.Ca,cal.Na,cal.K]).*100];
...
subplot(2,3,6);
scatter(X(:,1)*100,X(:,7)*100,25,DATA.T,'filled'); colormap('copper'); hold on
scatter(Xp(:,1)*100,Xp(:,7)*100,25,DATA.T);
...
for ic = nc
    ...
    DATA.X = [DATA.X;MAG(ic).OUT.OxideFract.pl4T(hasfsp,[cal.Si,cal.Al,cal.Ca,cal.Na,cal.K]).*100];
...
for ic = nc
    if isfield(MAG(ic).OUT.PhaseProps,'pl4T')
        hasmlt = MAG(ic).OUT.PhaseFractions.liq_wt>=0.001 & MAG(ic).OUT.PhaseFractions.sol_wt>=0.001 & MAG(ic).OUT.PhaseProps.bi(:,1)<=0.001;
        DATA.c_oxd  = [DATA.c_oxd ;MAG(ic).OUT.OxideFract.SYS(hasmlt,:).*100];
        DATA.cm_oxd = [DATA.cm_oxd;MAG(ic).OUT.OxideFract.liq(hasmlt,:).*100];
        DATA.cx_oxd = [DATA.cx_oxd;MAG(ic).OUT.OxideFract.sol(hasmlt,:).*100];
