...
if ~hold_state
	...
	text(lxc+0.01, lyc, [repmat('  ', length(labels), 1) labels],  'VerticalAlignment', 'Middle');
	
	% Plot bottom labels (no b - only a c)
	[lxb, lyb] = terncoords(majorticks, zerocomp, 1-majorticks); % fB = 1-fA
	text(lxb-0.015, lyb-0.1, labels, 'VerticalAlignment', 'Middle');
	
	% Plot left labels (no a - only c b)
	[lxa, lya] = terncoords(zerocomp, 1-majorticks, majorticks);
	text(lxa-0.085, lya+0.015, labels, 'VerticalAlignment', 'Middle');
