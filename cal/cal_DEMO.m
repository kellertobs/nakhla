...
cal.mem_oxd    = [ 43    0    0    0   57    0    0    0      % forsterite (ant) 
                   30    0    0   70    0    0    0    0      % fayalite (ant) 

                   44    0   36    0    0   20    0    0      % anorthite (ant)
                   69    0   19    0    0    0   12    0      % albite (ant)

                   54    0    3    3   21   19    0    0      % diopside (dps)
                   49    3    1   29    0   14    4    0      % augite (aug)

                    0   38    3   31   28    0    0    0      % ulvospinel (ulv)
                    0    6    1   93    0    0    0    0      % magnetite (mgt)

                  100    0    0    0    0    0    0    0      % quartz (qtz)
                    0    0    0    0    0    0    0  100 ];   % water (wat)
...
%               for  fay  ant  alb  dps  aug  ulv  mgt  qtz  wat
cal.rhox0   = [3200,4400,2700,2600,3300,3500,3930,4750,2500,1000]; % mem ref densities [kg/m3]
cal.rhof0   = 1000;                 % fluid ref density [kg/m3]

% specify three-phase coefficient model parameters
%              for  fay  ant  alb  dps  aug  ulv  mgt  qtz  wat
cal.etax0  = [1e18,1e18,1e17,1e17,1e19,1e19,1e17,1e17,1e19,1e0]; % mem ref viscosities [Pas]
