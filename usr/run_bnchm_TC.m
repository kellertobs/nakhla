...
DT = [8,4,2,1];  % test increasing time steps
figure(15); clf;

for dt = DT
    
% set run parameters
runID    =  'bnchm_TC_dt8';      % run identifier
...
M        =  256/dt;              % number of time steps to take
hr       =  3600;                % conversion seconds to hours
yr       =  12*hr;               % conversion seconds to years
tend     =  1*yr;                % end time for simulation [s]
dt       =  dt;                  % initial time step [s]
...
dEHdt = mean(abs(diff(hist.EH(2:end)))/dt);
dECdt = mean(abs(diff(hist.EC(2:end)))/dt);
dEVdt = mean(abs(diff(hist.EV(2:end)))/dt);

loglog(dt,dEHdt,'r+',0.99*dt,dECdt,'g+',1.01*dt,dEVdt,'b+','MarkerSize',8,'LineWidth',2); hold on; box on;

if dt == DT(1)
    loglog(DT,mean([dEHdt,dECdt,dEVdt]).*(DT./DT(1)).^1,'k-');  % plot linear trend for comparison
end
