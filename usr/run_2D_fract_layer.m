...
% set run parameters
runID    =  '2D_fract_layer';    % run identifier
...
T0       =  1070;                % temperature top layer [deg C]
T1       =  1130;                % temperature base layer [deg C]
dT       =  0;                   % amplitude of random noise [deg C]
c0       =  0.55;                % major component top layer [wt SiO2]
...
te1      =  1./te0;              % trace elements base layer [wt ppm]
dte      =  1e-3.*[1,1,-1,-1];   % trace elements random noise [wt ppm]
Kte      =  [0.01,0.1,3,10];     % trace elements partition coefficients
ir0      =  [1,0.76];            % isotope ratios top layer [delta]
ir1      =  [5,0.72];            % isotope ratios base layer [delta]
dir      =  [0.1,-0.001];        % isotope ratios random noise [delta]
...
tau_T    =  100*hr;              % wall cooling/assimilation time [s]
tau_a    =  100*hr;              % wall cooling/assimilation time [s]
...
BB       = [ 0.30, 0.15, 0.55; 0.49, 0.01, 0.50; 0.80, 0.08, 0.12; ];  % permission step locations
